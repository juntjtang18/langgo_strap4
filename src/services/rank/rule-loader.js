...
module.exports = ({ strapi }) => {
  ...
  const refresh = async () => {
    const [groupRules, levelRules, eventRules, groupRanks, levelTitlesByLocale, groupRankTitlesByLocale] = await Promise.all([
      strapi.entityService.findMany('api::rs-group-rule.rs-group-rule', {
        limit: 1,
        sort: { id: 'asc' },
      }),
      strapi.entityService.findMany('api::rs-level-rule.rs-level-rule', {
        limit: 1,
        sort: { id: 'asc' },
      }),
      strapi.entityService.findMany('api::rs-event-list.rs-event-list', {
        limit: 1000,
      }),
      strapi.entityService.findMany('api::rs-group-rank.rs-group-rank', {
      ...
      loadTitles('api::rs-level-title.rs-level-title', 'rs_level'),
      loadTitles('api::rs-group-rank-title.rs-group-rank-title', 'rs_group_rank'),
    ]);
  ...
  return {
    ...
    async findOrCreateLevel(levelNo) {
      ...
      const created = await strapi.entityService.create('api::rs-level.rs-level', {
