...
async function resolvePeriodMetrics(userid, snapshot) {
  ...
  const groupRule = await strapi.plugin('rank-system').service('rule-loader').loadGroupRule();
  const periodDays = groupRule?.period_days || 7;
  const snapshotDate = snapshot.record_date ? toUtcDate(snapshot.record_date) : new Date();
  const snapshotService = strapi.plugin('rank-system').service('snapshot');
...
module.exports = {
  async submitEvent(ctx) {
    ...
    const result = await strapi.plugin('rank-system').service('event-processor').processEvent({
      event_id,
      event_name,
      userid,
      payload: payload || {},
  ...
  async getMyLeaderboard(ctx) {
    ...
    const group = await strapi.entityService.findOne('plugin::rank-system.rs-group', membership.rs_group.id, {
      fields: ['id', 'group_no'],
      populate: {
        rs_group_rank: {
          fields: ['id', 'rank_no'],
    ...
    const members = await strapi.plugin('rank-system').service('user-group').listByGroup(membership.rs_group.id);
