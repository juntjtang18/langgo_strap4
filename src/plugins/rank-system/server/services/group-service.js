...
module.exports = ({ strapi }) => ({
  ...
  async getNextGroupNo(groupRuleId) {
    const groups = await strapi.entityService.findMany('plugin::rank-system.rs-group', {
  ...
  async createGroup(groupRuleId, groupRankId) {
    ...
    return strapi.entityService.create('plugin::rank-system.rs-group', {
      data: {
  ...
  async getCurrentMembersOfGroup(groupId) {
    const rows = await strapi.plugin('rank-system').service('user-group').listByGroup(groupId);
    return rows.map((row) => row.userid);
  },

  async countCurrentGroupMembers(groupId) {
    return strapi.plugin('rank-system').service('user-group').countByGroup(groupId);
  },
  ...
  async assignUserToRankedGroup(userid, username, groupRuleId, groupRankId, groupSize, periodPoints, groupRankTitle, visibleOnLadder = null) {
    ...
    if (existingMembership?.rs_group?.id) {
      const currentGroup = await strapi.entityService.findOne('plugin::rank-system.rs-group', existingMembership.rs_group.id, {
        populate: {
          rs_group_rank: {
            fields: ['id'],
          },
    ...
    const finalGroup = finalMembership?.rs_group?.id
      ? await strapi.entityService.findOne('plugin::rank-system.rs-group', finalMembership.rs_group.id)
      : targetGroup;
