...
module.exports = ({ strapi }) => {
  const rules = () => strapi.plugin('rank-system').service('rule-loader');
  const snapshots = () => strapi.plugin('rank-system').service('snapshot');
  ...
  const logEvent = async (eventData) =>
    strapi.entityService.create('plugin::rank-system.rs-event', {
      data: {
        event_id: eventData.event_id || null,
        event_name: eventData.event_name,
        userid: String(eventData.userid),
  ...
  const markHandled = (id, result) =>
    strapi.entityService.update('plugin::rank-system.rs-event', id, {
      data: {
        status: 'handled',
